...
handle_image() {
    ...
    case "${mimetype}" in
        ...
        image/*)
            ...
            exit 7;;
            # kitty +kitten icat --silent --transfer-mode=file --place="$width"x"$height"@0x0 "$path" && exit 6
            # ;;
        ## Video
        # video/*)
        ...
        application/pdf)
            pdftoppm -f 1 -l 1 \
                ...
                && exit 6 || exit 1;;
        # application/pdf)
        #     pdftoppm -f 1 -l 1 "${FILE_PATH}" "${IMAGE_CACHE_PATH%.*}" -png > /dev/null
        #     exit 6
        #     ;;
        #
